...
if (cartItem) {
	cartItem.forEach(item => {
		...
		let roundValueToMultipleOfFourDB = debounce(roundValueToMultipleOfFour, 500);
		amountInput.addEventListener("input",roundValueToMultipleOfFourDB );
		...
		/*   const choice = formToggle.querySelector('.form-toggle__item input:checked').value; */
			let _form = amountInput.closest('form.ms2_form'),
				_price = 0,
				_old_price = 0;
			...
			if (choice === "шт") {
				...
				_price = _form.querySelector('input[name="price_part"]').value.replace(/\D/g, "")
				_old_price = _form.querySelector('input[name="old_price_part"]').value.replace(/\D/g, "")

			} else {
				amountInput.value = Math.ceil(amountInput.value / step);
				_price = _form.querySelector('input[name="price_pack"]').value.replace(/\D/g, "")
				_old_price = _form.querySelector('input[name="old_price_pack"]').value.replace(/\D/g, "")
			}
			item.querySelector('.ms-price span').textContent = miniShop2.Utils.formatPrice(_price)
			// console.log(item.querySelector('.ms-price sna'))
			weChVar({
				key: $(_form).children('input[name="key"]').val(),
				price: _price,
				old_price: _old_price,
				discount_price: _price - _old_price,
				discount_cost: (_price - _old_price)*amountInput.value,
				count: amountInput.value,
				modif_name: choice,
				var_modif: $(_form).children('input[name="var_modif"]').val()
			})
		}
